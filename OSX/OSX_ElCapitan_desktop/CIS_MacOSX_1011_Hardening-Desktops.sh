#!/bin/bash
#
#CIS MacOSX 10.10 hardening script
#v1.1.0 - 04-02-2015
#author rnolette
#date 06/06/2016
#version 1.0
#description: This script will harden a system for compliance with the CIS V1.1.0 hardening guide for MacOSX
#####################################################################
...
#disabled since filevault is not required for servers
# echo "Please enter the password for the system admin user: "
# read system admin password
######################################################################
...
###############################
# echo "2.2 Date & Time"
systemsetup -f -setnetworktimeserver ntpserver.domain.com
...
# <key>Password</key>
# <string>$system admin password</string>
# </dict>
# </plist>
# EOM
...
# java -version
###############################
echo "2.11 Securely delete files as needed"
...
echo "3.1.1 Retain system.log for 30 or more days"
#/usr/bin/sed -i.bak 's/^>\ system\.log.*/>\ system\.log\ mode=640\ format=bsd\ rotate=seq\ ttl=30/' /etc/asl.conf
/usr/bin/sed -i.bak 's/^>\ system\.log.*/>\ system\.log\ mode=640\ format=bsd\ rotate=utc\ compress\ file_max=5M\ ttl=30/' /etc/asl.conf

echo "3.1.2 Retain appfirewall.log for 30 or more days"
#/usr/bin/sed -i.bak 's/^\?\ \[=\ Facility\ com.apple.alf.logging\]\ .*/\?\ \[=\ Facility\ com.apple.alf.logging\]\ file\ appfirewall.log\ rotate=seq\ ttl=30/' /etc/asl.conf
/usr/bin/sed -i.bak 's/^\?\ \[=\ Facility\ com.apple.alf.logging\]\ .*/\?\ \[=\ Facility\ com.apple.alf.logging\]\ file\ appfirewall.log\ mode=0640\ format=bsd\ rotate=utc\ compress\ file_max=5M\ ttl=30/' /etc/asl.conf

echo "3.1.3 Retain authd.log for 30 or more days "
#/usr/bin/sed -i.bak 's/^\*\ file\ \/var\/log\/authd\.log.*/\*\ file\ \/var\/log\/authd\.log\ mode=640\ format=bsd\ rotate=seq\ ttl=30/' /etc/asl/com.apple.authd
/usr/bin/sed -i.bak 's/^\*\ file\ \/var\/log\/authd\.log.*/\*\ file\ \/var\/log\/authd\.log\ mode=0640\ format=bsd\ rotate=utc\ compress\ file_max=5M\ ttl=30/' /etc/asl/com.apple.authd
...
/usr/bin/sed -i '' 's/^naflags:.*/flags:lo,ad,fd,fm,-all/' /etc/security/audit_control
/usr/bin/sed -i '' 's/^expire-after:.*/expire-after:100M/' /etc/security/audit_control
...
/usr/bin/sed -i.bak 's/^\*\ file\ \/var\/log\/install\.log.*/\*\ file\ \/var\/log\/install.log\ mode=0640\ format=bsd\ rotate=utc\ compress\ file_max=5M\ ttl=90/' /etc/asl/com.apple.install
##############################################################

#4 Network Configurations
echo "4 Network Configurations"
echo "4.1 Disable Bonjour advertising service (Scored)"
...
###############################
#echo "4.2 Enable Show Wi-Fi status in menu bar (Scored)"
###############################
#echo "4.3 Create network specific locations"
###############################
echo "4.4 Ensure http server is not running"
#stop web server
apachectl stop
#stop web server from auto start at boot
defaults write /System/Library/LaunchDaemons/org.apache.httpd Disabled -bool true
###############################
echo "4.5 Ensure ftp server is not running"
-s launchctl unload -w /System/Library/LaunchDaemons/ftp.plist
###############################
echo "4.6 Ensure nfs server is not running"
nfsd disable
rm /etc/export
##############################################################
...
###############################
echo "5.1.2 Check System Wide Applications for appropriate permissions"
...
###############################
echo "5.3 Reduce the sudo timeout period (Scored)"
...
EOM
###############################
#echo "5.4 Automatically lock the login keychain for inactivity"
...
###############################
#echo "5.5 Ensure login keychain is locked when the computer sleeps"
...
###############################
echo "5.6 Enable OCSP and CRL certificate checking"
...
###############################
echo "5.9 Require a password to wake the computer from sleep or screen saver - requires reboot"
defaults write com.apple.screensaver askForPassword -int 1
###############################
echo "5.10 Require an administrator password to access system-wide preferences"
...
#echo Done.
###############################
echo "5.11 Disable ability to login to another user's active and locked session"
...
#echo "5.12 Create a custom message for the Login Screen"
...
#echo "5.13 Create a Login window banner"
# since the systems will be on the domain, we are not doing this step
...
###############################
#echo "5.14 Do not enter a password-related hint"
# since the systems will be on the domain, we are not doing this step
# 	Remediation:
# 1. Open System Preferences
...
###############################
#echo "5.16 Secure individual keychain items"
...
#echo "5.17 Create specialized keychains for different purposes"
#small value add in security for large user inconvience.
# 	Remediation:
# 1. Open Utilities
# 2. Select Keychain Access
...
##############################################################
# ############secops applications
# #secops 1.1 - join system to domain
#
# #secops 1.2 - install bradford agent
# #secops 1.3 - install CBEP
# #secops 1.4 - install CBER
# #secops 1.5 - install splunk
# #secops 1.6 - modify hosts.allow
...
# #secops 1.7 - modify hosts.deny
# touch /etc/hosts.deny
# echo "ALL: ALL" >> /etc/hosts.deny
# /bin/chmod 644 /etc/hosts.deny
# #secops 1.8 - configure ssh
...
# EOM
# ##############################################################
# #secops 1.9 - configure security logging
